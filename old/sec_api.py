...
class SEC_API:
    ...
    def _session(self):
        """Disk-cached requests session for all SEC EDGAR calls.

        Responses are persisted to 'sec_cache' (SQLite) and reused across
        runs until cache_ttl seconds have elapsed (None = never expire).
        The in-memory _companyfacts layer above this avoids SQLite round-trips
        within the same Python session.
        """
        if self._requests_sess is None:
            from requests_cache import CachedSession
            self._requests_sess = CachedSession('sec_cache', expire_after=self.cache_ttl)
            self._requests_sess.headers.update(self.EDGAR_HEADERS)
        return self._requests_sess

    def _fetch_cik(self) -> str:
        if self._cik is not None:
            return self._cik
        r = self._session().get('https://www.sec.gov/files/company_tickers.json')
        r.raise_for_status()
